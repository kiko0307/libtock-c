...
{
  printf("APP: libtock_screen_command_write called with length=%u\n", length);

  syscall_return_t cval = command(DRIVER_NUM_SCREEN, 200, length, 0);

  printf("APP: syscall return value: type=%d, data[0]=%u, data[1]=%u, data[2]=%u\n",
         cval.type, cval.data[0], cval.data[1], cval.data[2]);
