...
//   if (err != RETURNCODE_SUCCESS)
//   {
//     printf("Could not register ipc %i ?\n", err);
//   }
...
//   if (client_count == 10 || len < 8)
//     return; // 需要 8 字节存储湿度和电压
...
//   // 调试信息
//   reading_count += 1;
//   printf("[Sensor] #%lu Voltage: %lu.%03luV Soil: %lu.%lu%%\n",
//          reading_count, *voltage_mv / 1000, *voltage_mv % 1000, soil / 10, soil % 10);
...
// {
//   uint32_t voltage_mv;
//   uint32_t moisture = take_measurement(reference_voltage, &voltage_mv);

//   // 将湿度数据和电压数据发送到 IPC 客户端
//   for (int i = 0; i < client_count; i++)
//   {
//     uint32_t *data_buf = (uint32_t *)clients[i].buffer;
//     data_buf[0] = moisture;
//     data_buf[1] = voltage_mv;
//     ipc_notify_client(clients[i].pid);
//   }
// }
...
int main(void)
{
  ...
  // 注册 IPC 服务
  err = ipc_register_service_callback("soil-moisture-sensor", ipc_client_registered, NULL);
  if (err != RETURNCODE_SUCCESS)
  {
    printf("Could not register IPC: %d\n", err);
    return -1;
  }
