...
returncode_t libtocksync_screen_get_pixel_format(libtock_screen_format_t *format)
{
  ...
  if (result_format.ret != RETURNCODE_SUCCESS)
    return result_format.ret;
...
returncode_t libtocksync_screen_get_rotation(libtock_screen_rotation_t *rotation)
{
  ...
  if (result_rotation.ret != RETURNCODE_SUCCESS)
    return result_rotation.ret;
...
returncode_t libtocksync_screen_fill(uint8_t *buffer, int buffer_len, size_t color)
{
  ...
  // Wait for the callback.
  yield_for(&result.fired);
  if (result.ret != RETURNCODE_SUCCESS)
    return result.ret;
...
returncode_t libtocksync_screen_write(uint8_t *buffer, int buffer_len, size_t length)
{
  ...
  // Debug: Print buffer content before allowing it to kernel
  printf("APP: Writing to screen, buffer address: %p, first byte: %d, buffer_len: %d, length: %zu\n", buffer, buffer[0], buffer_len, length);

  ret = libtock_screen_write(buffer, buffer_len, length, screen_cb_done);
  if (ret != RETURNCODE_SUCCESS)
  {
    printf("APP: libtock_screen_write failed with error %d\n", ret);
    return ret;
  }
  ...
  if (result.ret != RETURNCODE_SUCCESS)
  {
    printf("APP: Screen write callback returned error %d\n", result.ret);
    return result.ret;
  }
  ...
  if (ret < 0)
  {
    printf("APP: allow() failed with error %d\n", ret);
  }
  else
  {
    printf("APP: allow() succeeded, buffer address: %p, first byte: %d\n", buffer, buffer[0]);
    return ret;
  }
}
